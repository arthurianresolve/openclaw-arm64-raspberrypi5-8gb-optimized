...
| `blakecrosley.com/guides`                       | Public guide set for agent, media, iOS, design, and retrieval review vocabulary | clean-room review vocabulary for hooks, skills, MCP, memory, subagents, media prompt structure, iOS agent boundaries, and retrieval evaluation | copied guide text, tool-specific commands as source of truth, vendor or model claims without official docs, Python/FastAPI stack changes, or new dependencies |
...
### Guide set review rule

Public third-party guides can sharpen Excaliclaw review vocabulary, but they
are not implementation sources. Before adopting an idea from a guide set,
identify the local owner path, the changed invariant, the validation artifact,
and the source of truth for any vendor-specific claim. If the idea is about
retrieval quality, media prompting, or agent workflow efficiency, start with a
docs or skill update unless measured evidence shows runtime code needs to
change.
