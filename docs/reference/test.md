...
- `node scripts/test-projects.mjs --plan-json <targets...>` prints the compact routed Vitest plan without executing tests. Use it to inspect config selection, include-pattern routing, cache env keys, and heavy-check lock decisions before running expensive shards.
