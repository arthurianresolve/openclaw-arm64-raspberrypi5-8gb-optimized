...
- `OPENCLAW_TEST_PROFILE=pi5-8gb pnpm test` uses the Raspberry Pi 5 8 GB local profile: one test-project shard at a time, one Vitest worker by default, local-check policy enabled, longest-known shards first, a 420 second full-suite no-output watchdog, and the existing 300 second extension test timeout. Explicit `OPENCLAW_TEST_PROJECTS_SERIAL`, `OPENCLAW_VITEST_MAX_WORKERS`, or `OPENCLAW_TEST_WORKERS` values still override the profile defaults.
- `pnpm test:perf:arm64`: runs the Pi 5 8 GB profile and emits benchmark JSON with profile, host, worker, shard duration, failure, and no-output retry details after the normal test summary.
- `node scripts/test-projects.mjs --plan-json <targets...>` prints the compact routed Vitest plan without executing tests. Use it to inspect config selection, include-pattern routing, active test profile, cache env keys, and heavy-check lock decisions before running expensive shards.
- `node scripts/test-projects.mjs --benchmark-json <targets...>` executes the selected Vitest plan and writes benchmark JSON to stdout after the normal stderr progress and summary output.
- Test wrapper runs end with a short `[test] passed|failed|skipped ... in ...` summary. Vitest's own duration line stays the per-shard detail.
- Full, extension, and include-pattern shard runs update local timing data in `.artifacts/vitest-shard-timings.json`; later whole-config parallel runs use those timings to balance slow and fast shards, while sequential full-suite runs use the same data longest-first to surface expensive failures earlier. Include-pattern CI shards append the shard name to the timing key, which keeps filtered shard timings visible without replacing whole-config timing data. Set `OPENCLAW_TEST_PROJECTS_TIMINGS=0` to ignore the local timing artifact.
